...
__all__ = [
    'VGG',
    'vgg8', 'vgg8_bn', 'vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg16', 'vgg16_bn',
    'vgg8_pau', 'vgg8_bn_pau', 'vgg11_pau', 'vgg11_bn_pau', 'vgg13_pau', 'vgg13_bn_pau', 'vgg16_pau', 'vgg16_bn_pau',
    'vgg19_bn', 'vgg19',
]
...
class VGG(nn.Module):
    ...
    def __init__(self, cfg, batch_norm=False, num_classes=1000, af=nn.ReLU):
    ...
    @staticmethod
    def _make_layers(cfg, batch_norm=False, in_channels=3, af=nn.ReLU):
        layers = []
        for v in cfg:
            if v == 'M':
                ...
                if batch_norm:
                    layers += [conv2d, nn.BatchNorm2d(v), af()]
                else:
                    layers += [conv2d, af()]
                in_channels = v
        layers = layers[:-1]
...
def PAU_():
    return PAU(version='A', train_center=False)
...
def vgg8_pau(**kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = VGG(cfg['S'], af=PAU_, **kwargs)
    return model


def vgg8_bn_pau(**kwargs):
    """VGG 8-layer model (configuration "S")
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = VGG(cfg['S'], af=PAU_, batch_norm=True, **kwargs)
    return model


def vgg11_pau(**kwargs):
    """VGG 11-layer model (configuration "A")
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = VGG(cfg['A'], af=PAU_, **kwargs)
    return model


def vgg11_bn_pau(**kwargs):
    """VGG 11-layer model (configuration "A") with batch normalization"""
    model = VGG(cfg['A'], af=PAU_, batch_norm=True, **kwargs)
    return model


def vgg13_pau(**kwargs):
    """VGG 13-layer model (configuration "B")
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = VGG(cfg['B'], af=PAU_, **kwargs)
    return model


def vgg13_bn_pau(**kwargs):
    """VGG 13-layer model (configuration "B") with batch normalization"""
    model = VGG(cfg['B'], af=PAU_, batch_norm=True, **kwargs)
    return model


def vgg16_pau(**kwargs):
    """VGG 16-layer model (configuration "D")
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = VGG(cfg['D'], af=PAU_, **kwargs)
    return model


def vgg16_bn_pau(**kwargs):
    """VGG 16-layer model (configuration "D") with batch normalization"""
    model = VGG(cfg['D'], af=PAU_, batch_norm=True, **kwargs)
    return model


def vgg19_pau(**kwargs):
    """VGG 19-layer model (configuration "E")
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = VGG(cfg['E'], af=PAU_, **kwargs)
    return model


def vgg19_bn_pau(**kwargs):
    """VGG 19-layer model (configuration 'E') with batch normalization"""
    model = VGG(cfg['E'], af=PAU_, batch_norm=True, **kwargs)
    return model
